...
# RELEASE VERSION 1.2
#
# GCC ARM64 UEFI Bootloader Linux Compile Script for ARM64 Hosts
#
# by KNNSpeed
#
# =================================
...
CurDir=$PWD
GCC_FOLDER_NAME=gcc-linux-arm64
EFI_FOLDER_NAME=gnu-efi-3.0.9
BINUTILS_FOLDER_NAME=binutils-binaries
...
set -v
"$BINUTILS_FOLDER_NAME/bin/ld" -T$LinkerScript -nostdlib -s -shared -Bsymbolic --warn-common --no-undefined -ztext -znorelro -znow --defsym=EFI_SUBSYSTEM=0xa -znocombreloc -Map=output.map -o "program.so" @"objects.list"
set +v
...
# The PE file this makes is a little weird, so size doesn't understand it.
echo
echo Generating binary and Printing size information:
echo
"$BINUTILS_FOLDER_NAME/bin/objcopy" -j .text -j .sdata -j .data -j .dynamic -j .dynsym -j .rel -j .rela -j .rel.* -j .rela.* -j .rel* -j .rela* -j .reloc -O binary "program.so" "BOOTAA64.EFI"
"$BINUTILS_FOLDER_NAME/bin/size" "program.so"
echo
