...
//    size = 4 + IMAGE_SIZEOF_FILE_HEADER + IMAGE_SIZEOF_NT_OPTIONAL64_HEADER; // 264 bytes
    ...
    if(PEHeader.Signature == IMAGE_NT_SIGNATURE)
    {
      ...
      if(PEHeader.FileHeader.Machine == IMAGE_FILE_MACHINE_AA64 && PEHeader.OptionalHeader.Magic == IMAGE_NT_OPTIONAL_HDR64_MAGIC) // PE Headers have Signature, FileHeader, and OptionalHeader
      {
        // PE32+, Aarch64 version

#ifdef LOADER_DEBUG_ENABLED
        Keywait(L"AARCH64 PE32+ header passed.\r\n");
#endif
...
//        GoTimeStatus = BS->AllocatePages(AllocateMaxAddress, EfiLoaderData, pages, &AllocatedMemory);
      ...
      {
        GoTimeStatus = EFI_INVALID_PARAMETER;
        Print(L"Hey! 64-bit (Aarch64) only.\r\n");
        return GoTimeStatus;
      }
    }
    else
  ...
  else // Check if it's an ELF, if it's not a PE32+
  {
        ...
        for(i = 0; i < Numofprogheaders; i++) // Load sections into memory
        {
          ...
          Keywait(L"Found a PT_DYNAMIC section...\r\n");
            ...
            if( (Rela_table != (Elf64_Rela*)~0ULL) ) // Pointer found
            {
              ...
              {
                if(ELF64_R_TYPE(Rela_table[Rela_iter].r_info) == R_AARCH64_RELATIVE)
                {
#ifdef ELF_LOADER_DEBUG_ENABLED
                ...
                {
                  Print(L"Not an AARCH64 relative relocation. Other relocation types are not supported.\r\nUnsafe to continue because things will break (ELF).\r\n");
                  return EFI_LOAD_ERROR;
                }
#endif
              } // end for
    ...
    else
    {
      ...
      if(MACheader.magic == MH_MAGIC_64 && MACheader.cputype == CPU_TYPE_ARM64) // Aarch64 edition
      {
        // Big endian: 0xfeedfacf && little endian: 0x12000001

#ifdef LOADER_DEBUG_ENABLED
        Keywait(L"AARCH64 Mach64 header passed.\r\n");
#endif
...
*/
 ...
 //----------------------------------------------------------------------------------------------------------------------------------
  ...
  // UINTN is the largest uint type supported. For Aarch64, this is uint64_t
