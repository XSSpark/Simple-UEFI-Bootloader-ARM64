...
//
// Source Code:
//  https://github.com/KNNSpeed/Simple-UEFI-Bootloader-ARM64
//
//----------------------------------------------------------------------------------------------------------------------------------
// About this program:
//----------------------------------------------------------------------------------------------------------------------------------
//
// This program is an ARM64 bootloader for UEFI-based systems. It's like GRUB, but simpler! (Though it won't boot Linux, Windows, etc.)
...
*/
//
// This bootloader is primarily intended to enable programs to run "bare-metal," i.e. without an operating system, on ARM64 machines.
...
// an ARM64 computer like a microcontroller such as an Arduino, STM32F7, C8051, etc.
//
...
// based ARM64 machines. Also, starting from scratch on ARM64 is a very painful process (though not quite as intense as x86-64)...
// Especially when you can't use the screen to debug anything. This bootloader and corresponding loader parameter block mitigates such
...
//     folder. Put the desired bootloader binary in the BOOT folder, and name the binary BOOTAA64.EFI. This is just standard booting procedure:
...
//     default bootable file that UEFI firmware looks for is BOOTAA64.EFI in the directory \EFI\BOOT\, so you could just rename the bootloader
...
// select how you want your graphics output device(s) configured. See https://github.com/KNNSpeed/Simple-Kernel-ARM64 for an example, including
...
// application stored in the folder \EFI\BOOT\ with the filename BOOTAA64.EFI. In more inconvenient cases like these, it is probably easier
// to just boot from FAT32-formatted USB drives using the same \EFI\BOOT\BOOTAA64.EFI convention. If the UEFI firmware allows booting from
