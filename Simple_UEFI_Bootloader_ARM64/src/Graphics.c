...
// RaspberryPiPkg has GUID of {0xc5deae31, 0xfad2, 0x4030, {0x84, 0x1b, 0xcf, 0xc9, 0x64, 0x4d, 0x2c, 0x5b}} that works with this filter... But again that's too specific for this.
/*
              EFI_GUID PiGOPDeviceGuid = {0xc5deae31, 0xfad2, 0x4030, {0x84, 0x1b, 0xcf, 0xc9, 0x64, 0x4d, 0x2c, 0x5b}};
              VOID * ManagedInterface; // Need a throwaway pointer for OpenProtocol BY_DRIVER

              GOPStatus = BS->OpenProtocol(DevPathHandles[CntlrIndex], &PiGOPDeviceGuid, &ManagedInterface, Name2Handles[Name2DriverIndex], DevPathHandles[CntlrIndex], EFI_OPEN_PROTOCOL_BY_DRIVER);
              if(!EFI_ERROR(GOPStatus))
              {
                GOPStatus = BS->CloseProtocol(DevPathHandles[CntlrIndex], &PiGOPDeviceGuid, Name2Handles[Name2DriverIndex], DevPathHandles[CntlrIndex]);
                if(EFI_ERROR(GOPStatus))
                {
